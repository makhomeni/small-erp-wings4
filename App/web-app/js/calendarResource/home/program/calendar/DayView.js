...
Ext.ux.calendar.DayView = Ext.extend(Ext.ux.calendar.BasicView, {
	...
	generateHTML:function(data){        		
		...
		var port = '<div id="'+this.id+'-x-dayview-port" class="x-dayview-port x-dayview-inactive" unselectable="on" onselectstart="return false;">' +
			'<div id="'+this.id+'-x-dayview-body" class="x-dayview-body x-dayview-inactive" unselectable="on" onselectstart="return false;">' +
				'<table width="700px" cellspacing="0" cellpadding="0" border="0"><tbody>' +
					'<tr>' +
		...
		var pool = '<div id="'+this.id+'-x-dayview-pool" class="x-dayview-pool" unselectable="on" onselectstart="return false;">' +
			'<table width="715px" cellspacing="0" cellpadding="0" border="0"><tbody>' +
				'<tr>' +
	...
	initComponent:function(){        
		...
		for(var i = 0, len = this.dayNum; i < len; i++){
			this.daySet[this.daySet.length]	= sDate.add(Date.DAY, i);
		}        

		this.html = this.generateHTML();

		Ext.ux.calendar.DayView.superclass.initComponent.call(this);
        this.addEvents(
            'checklayout',
            'sizechanged',
            'afterresize',
	...
	_onPortMouseDownFn:function(e){
        ...
        if(0 == e.button){
            ...
            }else if(tgEl.hasClass('x-event-bottom-default')){
            ...
            }else if(!tgEl.hasClass('x-event-pin') && !tgEl.hasClass('x-event-content-link')){
                delete(this.preEl);
                var eEl = tgEl;
    ...
    initDetailCt:function(){
        ...
        this.detailCt = Ext.DomHelper.append(this.body, html, true);
        this.detailCt.setStyle('display', 'none');
    ...
    showEvents:function(eventSet, refresh){
         ...
         */
        if(1 == this.dayNum){
            var day = this.daySet[0].add(Date.DAY, 1).format('Y-m-d');
            var layout = glayout.getLayout(day, this, eventSet[day] || [], true, refresh);
            if(layout){
                layout.reLayout();
            }
