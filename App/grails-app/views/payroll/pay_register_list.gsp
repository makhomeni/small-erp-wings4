...
//
//             }
             %{--window.location="${createLink(controller: 'payroll',action: 'savePayRegister')}"--}%
//             payRegisterDataStore
      ...
      payRegisterDataStore = new Ext.data.Store({
            url: '${createLink(action: 'payRollJsonData')}',
            reader: new Ext.data.JsonReader({
                ...
                {name: 'payDate',type:'string',mapping:'payDate'},
                {name: 'salaryDue',type:'string',mapping:'salaryDue'},
                {name: 'startDate',type:'string',mapping:'startDate'},
                {name: 'endDay',type:'string',mapping:'endDay'},
                {name:'grossPay',type:'string',mapping:'grossPay'} ,
                {name: 'deductions',type:'string',mapping:'deductions'},
                {name: 'netPay',type:'string',mapping:'netPay'},
                {name: 'paymentMethod',type:'string',mapping:'paymentMethod'},
                {name: 'paymentIdentifier',type:'string',mapping:'paymentIdentifier'}
        ...
        payRegisterEditorGrid = new Ext.grid.EditorGridPanel({
            ...
            columns : [
                ...
                ,{
                    ...
                    dataIndex: 'paymentIdentifier',
                    header: 'Payment Identifier',
                    sortable: true,
                    width: 100
                }
                %{--,{--}%
    ...
    <div class="widget">
        ...
        <div id="list_user"  class="content scaffold-list" role="main" style="width: 940px;">
            ...
            &nbsp;&nbsp;&nbsp;
                ...
                <a id="generalSearch" class="button icon approve">Process Salary</a>
                <div style="margin-left: 800px;margin-top: -35px;">
                    <a id="excelExport" onclick="jasperExcelReportGenerator()" title="Generate Excel Salary Report"><img src="${resource(dir: 'images', file: 'excel.png')}" height="40px"></a>
                </div>
            ...
            <script>
                ...
                function jasperReportGenerator(){
                    if(Ext.get('searchById').getValue()!='' && Ext.get('toDate').getValue()!= '' && Ext.get('fromDate').getValue()!= ''){
                        ...
                        window.location = "../report/payRegisterReport?userCode=" + Ext.get('searchById').getValue()
                                + "&fromJobDate=" + Ext.get('fromDate').getValue()
                                + "&toJobDate=" + Ext.get('toDate').getValue()
                                + "&_name=" + "pay_register"
                                + "&_file=" + "salarySheetReport.jasper"
                                + "&_format=PDF"
                    }
                ...
                function jasperExcelReportGenerator(){
                    if(Ext.get('searchById').getValue()!='' && Ext.get('toDate').getValue()!= '' && Ext.get('fromDate').getValue()!= ''){

                        window.location = "../report/payRegisterExcelReport?userCode=" + Ext.get('searchById').getValue()
                                + "&fromJobDate=" + Ext.get('fromDate').getValue()
                                + "&toJobDate=" + Ext.get('toDate').getValue()
                                + "&_name=" + "pay_register"
                                + "&_file=" + "salarySheetExcelReport.jasper"
                                + "&_format=XLS"
                    }
                    else if(Ext.get('toDate').getValue()!= '' && Ext.get('fromDate').getValue()!= ''){
