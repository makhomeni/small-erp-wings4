...
Ext.ux.calendar.MonthView = Ext.extend(Ext.ux.calendar.BasicView, {
	...
	showMenu:function(El){
        if(this.menu){
            this.menu.bindEl = El;
            this.menu.show(Ext.get(El.dom.parentNode), this.menuAlign);
        }
        return this;
    },
	...
	generateHTML:function(data){
		...
		this.viewerTpl = new Ext.XTemplate(
            '<div id="'+this.id+'-x-monthview-viewer">' +
                '<table width="200px" cellspacing="0" cellpadding="0" border="0" unselectable="on" onselectstart="return false;"><tbody><tr>' +
                    '<td class="x-monthview-lefter">' +
                        '<div id="'+this.id+'-x-monthview-lefter">' +
                            '<tpl for=".">' +
                                '<div class="x-monthview-lefter-inner '+this.id+'-x-monthview-row">' +
                                    '<u><b id="'+this.id+'-x-monthview-week-{idx}-0" class="x-monthview-lefter-inner-b">{week}</b></u>' +
                                '</div>' +
                            '</tpl>' +
                        '</div>' +
                    '</td>' +
                    '<td class="x-monthview-port-td">' +
                        '<div id="'+this.id+'-x-monthview-port" class="x-monthview-port">' +
                            '<tpl for=".">' +
                                '<div class="'+this.id+'-x-monthview-row x-monthview-row" unselectable="on">' +
                                    '<table class="x-monthview-bg" width="100%" height="100%" cellspacing="0" cellpadding="0" border="0"><tbody>' +
                                        '<tr>' +
                                            '<tpl for="arr">' +
                                                '<td id="'+this.id+'-x-monthview-viewer-{parent.idx}-{idx}" class="'+this.id+'-x-monthview-viewer-col-{idx} x-monthview-viewer-cell">&nbsp;</td>' +
                                            '</tpl>' +
                                        '</tr>' +
                                    '</tbody></table>' +
                                    '<table id="'+this.id+'-x-monthview-ct-{idx}" class="x-monthview-ct" width="100%" cellspacing="0" cellpadding="0" border="0"><tbody>' +
                                        '<tr>' +
                                            '<tpl for="arr">' +
                                                '<td id="'+this.id+'-x-monthview-viewer-title-{parent.idx}-{idx}" class="'+this.id+'-x-monthview-viewer-col-{idx} x-monthview-viewer-title">' +
                                                    '<div>' +
                                                        '<table width="100%" height="100%" border="0"><tbody><tr>' +
                                                        '<td>' +
                                                            '<u id="'+this.id+'-x-monthview-viewer-link-{parent.idx}-{idx}" class="x-monthview-viewer-link"><b class="x-monthview-viewer-link-b">{day}</b></u>' +
                                                        '</td>' +
                                                        (this.ehandler.readOnly?'':
                                                        '<td class="x-monthview-viewer-tool">' +
                                                            '<img id="'+this.id+'-x-monthview-tool-add-{parent.idx}-{idx}" ' +
                                                                'class="x-monthview-tool-add" src="'+Ext.BLANK_IMAGE_URL+'"></img>' +
                                                            '<img id="'+this.id+'-x-monthview-tool-drop-{parent.idx}-{idx}" ' +
                                                                'class="x-monthview-tool-drop" src="'+Ext.BLANK_IMAGE_URL+'"></img>' +
                                                        '</td>') +
                                                    '</tr></tbody></table>' +
                                                    '</div>' +
                                                '</td>' +
		...
		var days = '';
		for(var j = 0; j < this.dayNum; j++){			
			days += '<td class="'+this.id+'-x-monthview-viewer-col-'+j+' x-dayview-header-days">' +
                '<div>' +
                    '<b style="line-height:15px;">'+Ext.ux.calendar.Mask.MonthView['dayPre']+this.daySet[j].format('l')+'</b>' +
                '</div>' +
			'</td>';
		}
		var header = '<div id="'+this.id+'-x-monthview-header" class="x-monthview-header" unselectable="on">' +
			'<table width="700px" height="100%" cellspacing="0" cellpadding="0" border="0"><tbody>' +
				'<tr>'+days+'</tr>' +
			'</tbody></table>' +
		'</div>';
	...
	setToday:function(){
		...
		for(var i = 0, len = this.daySet.length; i < len; i++){
			...
			if(day == today){
				...
				var tEl = Ext.get(this.id+'-x-monthview-viewer-title-'+x+'-'+y);
				if(tEl){
					tEl.setStyle('background-color', 'rgb(235,235,235)');
				}                
            }else{
                var x = Math.floor(i/this.dayNum);
				var y = i%this.dayNum;
				var tEl = Ext.get(this.id+'-x-monthview-viewer-title-'+x+'-'+y);
				if(tEl){
					tEl.setStyle('background-color', 'rgb(241,244,250)');
				}               
            }
		}
	},
	...
	initComponent:function(){
		...
		for(var i = 0; i < this.weekNum; i++){
			for(var j = 0; j < this.dayNum; j++){
				this.daySet[this.daySet.length] = sDate.add(Date.DAY, i*this.shiftDay+j);
			}
        }
		...
		Ext.ux.calendar.MonthView.superclass.initComponent.call(this);
        this.addEvents(
            'checklayout',
            'afterresize',
            'beforeremoteload',
    ...
    _onMouseUpFn:function(e){
        if(!this.dragging){
            if(this.startPos){
                ...
                eh.prepareLegend(Ext.get(this.id+'-x-monthview-viewer-title-'+epos.x+'-'+epos.y), spos, epos, this);
            }
        }
    },   
    ...
    alignDetailCt:function(){
        if(this.detailing){
            ...
            var tfh = this.detailTitle.getHeight()+this.detailFoot.getHeight()+20;
            var h = events.length*17;
            var bh = this.port.getBottom()-tEl.getTop()-tfh, th = cEl.getBottom()-this.port.getTop()-tfh;
    ...
    showDetails:function(day){
        ...
        this.detailTitle.dom.innerHTML = '<u id="'+Ext.id()+'-x-monthview-viewer-link-'+x+'-'+y+'" class="x-monthview-viewer-link">' +
            '<b class="x-monthview-viewer-link-b">'+day+'</b></u>';
    ...
    _onPortMouseDownFn:function(e){
        ...
        if(tgEl.hasClass('x-event-more')){
            ...
            if(this.detailing){
                if(!(tgEl.hasClass('x-event-detail-ct') || tgEl.parent('.x-event-detail-ct'))){
            ...
            if(!this.createByDblclick && !this.ehandler.readOnly){
                if(!tgEl.hasClass('x-monthview-tool-drop') && !tgEl.hasClass('x-monthview-viewer-link-b')){
                    var eEl;
    ...
    _onPortMouseOverFn:function(e){
        ...
        if(tgEl.hasClass('x-monthview-tool-drop') || tgEl.hasClass('x-monthview-tool-add')){
    ...
    addLegend:function(pos){
        ...
        this.startPos = null;
        eh.prepareLegend(Ext.get(this.id+'-x-monthview-viewer-title-'+pos.x+'-'+pos.y), pos, epos, this);
    },
    ...
    _onPortClickFn:function(e){
        ...
        if(tgEl.hasClass('x-event-detail-tool-close')){
            this.fireEvent('canceldetail');
        }else if(tgEl.hasClass('x-monthview-viewer-link-b')){
            ...
            if(tgEl.hasClass('x-monthview-tool-add')){
            ...
            }else if(tgEl.hasClass('x-monthview-tool-drop')){
                this.showMenu(tgEl);
            }else{
                if(tgEl.hasClass('x-whole-title-b')){
                    titleEl = tgEl;
                }else{
                ...
                if(titleEl){
                    ...
                    if(tgEl.hasClass('x-legend-title-b')){
                        titleEl = tgEl;
                    }else{
    ...
    initDetailCt:function(){
        ...
        this.detailCt = Ext.DomHelper.append(this.port, html, true);
        this.detailCt.setStyle('display', 'none');
        this.detailTitle = this.detailCt.child('.x-event-detail-title-td');
        this.detailViewer = this.detailCt.child('.x-event-detail-viewer');
        this.detailFoot = this.detailCt.child('.x-event-detail-foot-text');
    },
    ...
    initDragZone:function(bindEl){
        ...
        bindEl.dragzone = new Ext.dd.DragZone(bindEl, {
            ...
            endDrag:function(e){
                ...
                var oevent;
                if(event.day == event.eday && (0 != event.startRow || cview.rowCount != event.endRow)){
                ...
                if(event.day != day){
                    event.eday = date.add(Date.DAY, dnum).format('Y-m-d');
                    event.day = day;
    ...
    viewWeekend:function(display){
        if(1 == this.startDay){
            Ext.util.CSS.updateRule('.'+this.id+'-x-monthview-viewer-col-6', 'display', display);
            Ext.util.CSS.updateRule('.'+this.id+'-x-monthview-viewer-col-5', 'display', display);
        }else{
            Ext.util.CSS.updateRule('.'+this.id+'-x-monthview-viewer-col-0', 'display', display);
            Ext.util.CSS.updateRule('.'+this.id+'-x-monthview-viewer-col-6', 'display', display);
        }
    },
    ...
    recalculateSize:function(bw, bh){
    	...
    	if(typeof bh == 'number'){
            ...
            var th = Ext.get(this.id+'-x-monthview-viewer-title-0-1').getHeight();
            this.lineNum = Math.floor((this.ch-th)/this.legendHeight);            
           	Ext.util.CSS.updateRule('.'+this.id+'-x-monthview-row', 'height', Math.ceil(this.ch)+'px');
       	}
    },
    ...
    onPasteFn:function(item){
        if(this.cpFlag){
            ...
            eh.ds.changeDay(oday, tday, function(backObj){
                 ...
                 * delete oday wlist
                 */
                var wlist = gLayout.deleteDayFromWholeList(oday, oday, keep);
                ...
                /*
                 * if it's copy, backend will return new id, here need replace the old id with new ones
                 */
                ...
                if(!keep){
                    /*
                     * if it's cut, then need remove the elist in old layout
                     */
	...
	refreshDate:function(){
		...
		for(var i = 0; i < this.weekNum; i++){
            for(var j = 0; j < this.dayNum; j++){
                var titleEl = Ext.get(this.id+'-x-monthview-viewer-link-'+i+'-'+j);
                if(titleEl){
                    var day = this.daySet[i*this.dayNum+j].format(this.dayFormat);
                    titleEl.dom.innerHTML = '<b class="x-monthview-viewer-link-b">'+day+'</b>';
                }
            }
    ...
    locateDay:function(day){
        ...
        for(var i = 0; i < this.weekNum; i++){
            for(var j = 0; j < this.dayNum; j++){                
                ...
                this.daySet[this.daySet.length] = fd.add(Date.DAY, offset);
            }
        }        
    },
    ...
    recalculateWeek:function(fdate){
        if(1 == this.startDay){
            ...
            if(7 == n && 30 <= fdate.getDaysInMonth()){
                this.rowNum = 6;
            }else if(6 == n && 31 == fdate.getDaysInMonth()){
                this.rowNum = 6;
            }else if(1 == n && 28 == fdate.getDaysInMonth()){
                this.rowNum = 4;
            }else{
                this.rowNum = 5;
            }
            ...
            if(6 == w && 30 <= fdate.getDaysInMonth()){
                this.rowNum = 6;
            }else if(5 == w && 31 == fdate.getDaysInMonth()){
                this.rowNum = 6;
            }else if(0 == n && 28 == fdate.getDaysInMonth()){
                this.rowNum = 4;
            }else{
                this.rowNum = 5;
            }
    ...
    isShift:function(startDate, endDate){
        ...
        day2 = this.daySet[this.daySet.length-1].format('Y-m-d');        
        if(day1 > day2){
            minmax = day2;
        }else{
            minmax = day1;
